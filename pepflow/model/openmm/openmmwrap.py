...
from pepflow.model.openmm.openmm_base import _BridgeEnergy, _Bridge
...
class OpenMMBridge(_Bridge):
    ...
    def evaluate(
        ...
        if batch.is_cuda:
            batch = batch.cpu()
        batch_array = batch.detach().numpy()
        
        # assert correct number of positions
        assert batch_array.shape[1] == self._openmm_system.getNumParticles()
        ...
        forces = (
            torch.tensor(forces).to(batch)
            if evaluate_force else None
        )
        new_positions = (
            torch.tensor(new_positions).to(batch)

...
from pepflow.model import sde_lib
from pepflow.utils.tensor_utils import get_trace_computation_tensors
  ...
  def ode_sampler(model, z=None):
    ...
    with torch.no_grad():
      ...
      if not multiple_points:
          ...
          x = torch.tensor(solution.y).reshape(shape_).to(device).type(torch.float32).transpose(-1, -2).transpose(-2, -3)
  ...
  def ode_sampler(model, z=None):
    ...
    if only_likelihood:
        ...
        solution, delta_logp  = odeint(model, (x, init_logp), 
                times, rtol=rtol, atol=atol,  method="scipy_solver", options={"solver":"RK23"}, adjoint_method="euler", adjoint_options={"step_size":5e-2})
...
def get_sampling_likelihood_fn(sde, shape, inverse_scaler, device='cuda', hutchinson_type='Rademacher',
  ...
  def likelihood_fn(model, data=None, reverse=True):
    ...
    with torch.no_grad():
      ...
      delta_logp = from_flattened_numpy(zp[-shape[0]*shape[1]:], (shape[0],shape[1],)).to(data.device).type(torch.float32)
